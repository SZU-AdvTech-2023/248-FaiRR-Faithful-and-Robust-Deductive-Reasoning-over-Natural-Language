...
class FaiRRRuleSelector(BaseModel):
	def __init__(self, arch='roberta_large', train_batch_size=16, eval_batch_size=16, accumulate_grad_batches=1, learning_rate=1e-5, max_epochs=5,\
					optimizer='adamw', adam_epsilon=1e-8, weight_decay=0.0, lr_scheduler='linear_with_warmup', warmup_updates=0.0, freeze_epochs=-1, gpus=2,\
					accelerator='ddp',hf_name='roberta-large', cls_dropout=0.1):
		super().__init__(train_batch_size=train_batch_size, max_epochs=max_epochs, gpus=gpus, accelerator='ddp') # 修改了gpus, accelerator, precision参数
		...
		self.p.accelerator             = accelerator
	...
	def run_step(self, batch, split):
		...
		if split == 'train':
			...
			for metric in perf_metrics.keys():
				self.log(f'train_{metric}_step', perf_metrics[metric], prog_bar=True, sync_dist=True) # 添加了sync_dist=True参数
		else:
			self.log(f'{split}_loss_step', loss.item(), prog_bar=True, sync_dist=True) # 添加了sync_dist=True参数
			for metric in perf_metrics.keys():
				self.log(f'{split}_{metric}_step', perf_metrics[metric], prog_bar=True, sync_dist=True) # 添加了sync_dist=True参数

		return {'loss': loss, 'preds': preds, 'targets': targets, 'token_mask': token_mask}

	def aggregate_epoch(self, outputs, split):
		preds        = torch.cat([x['preds'].reshape(-1) for x in outputs], dim=0) # 添加了dim=0参数
		targets      = torch.cat([x['targets'].reshape(-1) for x in outputs], dim=0) # 添加了dim=0参数
		token_mask   = torch.cat([x['token_mask'].reshape(-1) for x in outputs], dim=0) # 添加了dim=0参数
		perf_metrics = self.calc_perf_metrics(preds, targets, token_mask)

		if split == 'train':
			self.log(f'train_loss_epoch', self.trainer.callback_metrics['train_loss_step'], prog_bar=True) # 使用了self.trainer.callback_metrics
			for metric in perf_metrics.keys():
				self.log(f'train_{metric}_epoch', perf_metrics[metric], prog_bar=True)
		else:
			self.log(f'{split}_loss_epoch', self.trainer.callback_metrics[f'{split}_loss_step'], prog_bar=True, sync_dist=True) # 使用了self.trainer.callback_metrics
			for metric in perf_metrics.keys():
				self.log(f'{split}_{metric}_epoch', perf_metrics[metric], prog_bar=True, sync_dist=True)

		return {'loss': self.trainer.callback_metrics[f'{split}_loss_step'], 'preds': preds, 'targets': targets, 'token_mask': token_mask}
	...
	def configure_optimizers(self):
		...
		return [optimizer], [scheduler], {'amp_backend': 'native'} # 添加了amp_backend参数
